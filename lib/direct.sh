...
edit_direct_server_listen_port() {
  local service_file="$1"
  local current_listen_port=$(grep -oP '--addr 0\.0\.0\.0:\K[0-9]+' "$service_file")
  echo -e "Current listen port: ${WHITE}$current_listen_port${RESET}"
  ...
  case $choice in
    ...
    2)
      ...
      print_success "Listen port removed."
      ;;
  esac
}

manage_direct_server_tunnel_ports() {
  local service_file="$1"
  manage_server_tunnel_ports "$service_file"
}

edit_direct_server_tcp_port() {
  edit_server_tcp_upstream_port "$1"
}

edit_direct_server_udp_port() {
  edit_server_udp_upstream_port "$1"
}

edit_direct_server_ports_action() {
  local service_file="/etc/systemd/system/trusttunnel-direct.service"
  if [ ! -f "$service_file" ]; then
    echo -e "${RED}❌ trusttunnel-direct.service not found. Please add direct server first.${RESET}"
    echo -e "${YELLOW}Press Enter to return to previous menu...${RESET}"
    read -p ""
    return
  fi
  ...
  while true; do
    ...
    echo ""
    draw_line "$CYAN" "=" 40
    echo -e "${CYAN}        ✏️ Edit Direct Server Ports${RESET}"
    draw_line "$CYAN" "=" 40
    echo ""
    ...
    case $choice in
      1)
        edit_direct_server_listen_port "$service_file"
        ;;
      2)
        manage_direct_server_tunnel_ports "$service_file"
        ;;
      3)
        edit_direct_server_tcp_port "$service_file"
        ;;
      4)
        edit_direct_server_udp_port "$service_file"
        ;;
      5)
        sudo systemctl daemon-reload
        sudo systemctl restart trusttunnel-direct.service
        break
        ;;
    esac
    echo ""
...
edit_direct_client_ports_action() {
  ...
  echo ""
  draw_line "$CYAN" "=" 40
  echo -e "${CYAN}        ✏️ Edit Direct Client Ports${RESET}"
  draw_line "$CYAN" "=" 40
  echo ""

  mapfile -t services < <(systemctl list-unit-files --full --no-pager | grep '^trusttunnel-direct-client-' | awk '{print $1}' | sed 's/.service$//')
  ...
  echo -e "${CYAN}📋 Please select a direct client:${RESET}"
  ...
  print_success "Direct client ports updated for '$selected_service'"
  echo ""
  echo -e "${YELLOW}Press Enter to return to previous menu...${RESET}"
  read -p ""
}
